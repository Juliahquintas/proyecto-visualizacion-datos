  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        sliderInput(
          "anio_sel_des",         # ID del input
          "Año",                  # Etiqueta que verá el usuario
          min = min(anios),       # Año mínimo disponible
          max = max(anios),       # Año máximo disponible
          value = max(anios),     # Año seleccionado por defecto
          step = 1,               # Para que se mueva año por año
          animate = TRUE           # Opcional: añade un botón de "play" para recorrer años automáticamente
        ),
