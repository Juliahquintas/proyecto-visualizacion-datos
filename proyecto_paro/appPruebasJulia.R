...
ui <- fluidPage(
  ...
  sidebarLayout( 
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        condition = "input.tabs_main == 'tab_relacion'",
        h4("Filtros Balance del Mercado Laboral"),
        radioButtons("nivel_agregacion", "Nivel de Agregación:",
                    choices = c("Provincial" = "provincia", 
                                "CCAA" = "ccaa"),
                    selected = "provincia"),
        
        # Este control se mantiene para filtrar por CCAA específica
        selectInput("ccaa_rel", "Filtrar por CCAA:", 
                    choices = c("Todas" = "ALL"), 
                    selected = "ALL"),
        ...
        # Nuevo: Seleccionar año específico o rango
        selectInput("anio_balance", "Año de análisis:",
                    choices = 2010:2025,
                    selected = 2024),
        
        checkboxInput("normalizar_poblacion", "Normalizar por población", value = TRUE),
        
        helpText("Balance = Contratos - Paro (valores positivos indican mercado en crecimiento)")
      ),
    ...
    mainPanel(
      tabsetPanel(id = "tabs_main",
        ...
        tabPanel(
            "Distribución y desigualdades demograficas del empleo",
        ...
        tabPanel(
          "Balance del Mercado Laboral",  # Cambiar este título
          value = "tab_relacion",
          br(),
          plotOutput("plot_relacion", height = "600px"),
          uiOutput("estadisticas_relacion")
...
server <- function(input, output, session) {
  ...
  # En el observer que actualiza las CCAA, añadir:

  # EN EL SERVER, REEMPLAZAR desde la línea que comienza con:
# "##########################################################"
# "## === RELACIÓN PARO (t) vs CONTRATOS (t + k) "
# HASTA el output$estadisticas_relacion

##########################################################
## === BALANCE DEL MERCADO LABORAL (Contratos vs Paro)
##########################################################

# 1. Datos para el balance
  datos_balance <- reactive({
    df <- datos_base()
    req(df)
    
    anio_target <- as.numeric(input$anio_balance)
    
    # Filtrar por año
    df_filtrado <- df %>%
      filter(anio == anio_target)
    
    # Preparar datos de paro (promedio mensual)
    paro_data <- df_filtrado %>%
      filter(metrica == "paro", sector != "Total", sector != "Otros") %>%
      group_by(anio, cod_prov, Provincia, `Comunidad Aut`, sector) %>%
      summarise(paro_promedio = mean(valor, na.rm = TRUE), .groups = "drop")
    
    # Preparar datos de contratos (suma anual)
    contratos_data <- df_filtrado %>%
      filter(metrica == "contratos", sector != "Total", sector != "Otros") %>%
      group_by(anio, cod_prov, Provincia, `Comunidad Aut`, sector) %>%
      summarise(contratos_total = sum(valor, na.rm = TRUE), .groups = "drop")
    
    # Combinar paro y contratos
    balance_data <- full_join(
      paro_data,
      contratos_data,
      by = c("anio", "cod_prov", "Provincia", "Comunidad Aut", "sector")
    ) %>%
      mutate(
        paro_promedio = replace_na(paro_promedio, 0),
        contratos_total = replace_na(contratos_total, 0),
        balance_absoluto = contratos_total - paro_promedio
      )
    ...
    if (input$normalizar_poblacion) {
      ...
      balance_data <- balance_data %>%
        left_join(
          df_pob %>% filter(anio == anio_target),
          by = c("cod_prov", "anio")
        ) %>%
        mutate(
          paro_rate = (paro_promedio / poblacion_total) * 1000,
          contratos_rate = (contratos_total / poblacion_total) * 1000,
          balance_rate = contratos_rate - paro_rate
        ) %>%
        filter(!is.na(balance_rate), is.finite(balance_rate))
    } else {
      balance_data <- balance_data %>%
        mutate(
          paro_rate = paro_promedio,
          contratos_rate = contratos_total,
          balance_rate = balance_absoluto
        )
    }
    ...
    if (input$ccaa_rel != "ALL") {
      balance_data <- balance_data %>% 
        filter(`Comunidad Aut` == input$ccaa_rel)
    }
    
    if (input$sector_rel != "Todos") {
      balance_data <- balance_data %>% 
        filter(sector == input$sector_rel)
    }
    
    return(balance_data)
  })

  # 2. Preparar datos para visualización según nivel de agregación
  datos_balance_plot <- reactive({
    df <- datos_balance()
    req(nrow(df) > 0)
    
    if (input$nivel_agregacion == "ccaa") {
      # Agregar a nivel CCAA
      df_agregado <- df %>%
        group_by(`Comunidad Aut`, sector) %>%
        summarise(
          balance_rate = mean(balance_rate, na.rm = TRUE),
          n_provincias = n_distinct(Provincia),
          .groups = "drop"
        ) %>%
        rename(region = `Comunidad Aut`) %>%
        arrange(balance_rate)
      
      # Ordenar por balance
      nivel_orden <- df_agregado %>%
        filter(sector == ifelse(input$sector_rel == "Todos", "Servicios", input$sector_rel)) %>%
        arrange(balance_rate) %>%
        pull(region)
      
      df_agregado$region <- factor(df_agregado$region, levels = nivel_orden)
      
    } else {
      # Mantener nivel provincial
      df_agregado <- df %>%
        rename(region = Provincia) %>%
        arrange(balance_rate)
      
      # Ordenar provincias dentro de su CCAA
      if (input$ccaa_rel == "ALL") {
        df_agregado <- df_agregado %>%
          mutate(region = factor(region, 
                                levels = unique(df_agregado$region[order(df_agregado$balance_rate)])))
      } else {
        df_agregado$region <- factor(df_agregado$region, 
                                    levels = unique(df_agregado$region[order(df_agregado$balance_rate)]))
      }
    }
  ...
  output$plot_relacion <- renderPlot({
    df <- datos_balance_plot()
    req(nrow(df) > 0)
    
    # Determinar si mostrar todos los sectores o solo uno
    if (input$sector_rel == "Todos") {
      # Gráfico de barras agrupadas por sector
      p <- ggplot(df, aes(x = region, y = balance_rate, fill = sector)) +
        geom_bar(stat = "identity", position = position_dodge(width = 0.8), 
                width = 0.7) +
        scale_fill_brewer(palette = "Set2", name = "Sector") +
        labs(
          title = paste("Balance del Mercado Laboral por", 
                      ifelse(input$nivel_agregacion == "ccaa", "CCAA", "Provincia"),
                      "- Año", input$anio_balance),
          subtitle = ifelse(input$ccaa_rel != "ALL", 
                          paste("CCAA:", input$ccaa_rel), 
                          "Todas las CCAA"),
          x = ifelse(input$nivel_agregacion == "ccaa", "Comunidad Autónoma", "Provincia"),
          y = ifelse(input$normalizar_poblacion,
                    "Balance (Contratos - Paro) por 1000 hab.",
                    "Balance (Contratos - Paro) Absoluto")
        )
    } else {
      # Gráfico de barras simples para un sector
      p <- ggplot(df, aes(x = region, y = balance_rate, 
                        fill = ifelse(balance_rate >= 0, "Positivo", "Negativo"))) +
        geom_bar(stat = "identity", width = 0.7) +
        scale_fill_manual(values = c("Positivo" = "#2E8B57", "Negativo" = "#CD5C5C"),
                          name = "Balance") +
        labs(
          title = paste("Balance del Mercado Laboral - Sector", input$sector_rel),
          subtitle = paste("Año", input$anio_balance, "|",
                          ifelse(input$nivel_agregacion == "ccaa", "CCAA", "Provincia"),
                          ifelse(input$ccaa_rel != "ALL", paste("-", input$ccaa_rel), "")),
          x = ifelse(input$nivel_agregacion == "ccaa", "Comunidad Autónoma", "Provincia"),
          y = ifelse(input$normalizar_poblacion,
                    "Balance (Contratos - Paro) por 1000 hab.",
                    "Balance (Contratos - Paro) Absoluto")
    ...
    p <- p +
      ...
      scale_y_continuous(labels = if(input$normalizar_poblacion) scales::number_format(accuracy = 0.1) 
                        else scales::comma_format())
    
    # Añadir etiquetas de valor si hay pocas barras
    if (nrow(df) <= 20) {
      p <- p + geom_text(aes(label = round(balance_rate, 1)),
                        position = position_dodge(width = 0.8),
                        vjust = ifelse(df$balance_rate >= 0, -0.5, 1.2),
                        size = 3)
    }
  ...
  output$estadisticas_relacion <- renderUI({
    ...
    stats <- df %>%
      summarise(
        n_registros = n(),
        balance_promedio = mean(balance_rate, na.rm = TRUE),
        balance_mediano = median(balance_rate, na.rm = TRUE),
        prop_positivos = mean(balance_rate > 0, na.rm = TRUE) * 100,
        mejor_region = if(input$nivel_agregacion == "ccaa") {
          df %>% arrange(desc(balance_rate)) %>% slice(1) %>% pull(`Comunidad Aut`)
        } else {
          df %>% arrange(desc(balance_rate)) %>% slice(1) %>% pull(Provincia)
        },
        peor_region = if(input$nivel_agregacion == "ccaa") {
          df %>% arrange(balance_rate) %>% slice(1) %>% pull(`Comunidad Aut`)
        } else {
          df %>% arrange(balance_rate) %>% slice(1) %>% pull(Provincia)
        }
      )
    
    tags$div(
      class = "well",
      h4("Estadísticas del Balance del Mercado Laboral:"),
      tags$ul(
        tags$li(paste("Año analizado:", input$anio_balance)),
        tags$li(paste("Número de registros:", stats$n_registros)),
        tags$li(paste("Balance promedio:", round(stats$balance_promedio, 2),
                    ifelse(input$normalizar_poblacion, "por 1000 hab.", ""))),
        tags$li(paste("Porcentaje con balance positivo:", round(stats$prop_positivos, 1), "%")),
        tags$li(paste("Mejor desempeño:", stats$mejor_region)),
        tags$li(paste("Peor desempeño:", stats$peor_region))
      ),
      tags$br(),
      tags$div(
        ...
        tags$ul(
          tags$li("Balance > 0: Más contratos que parados (mercado en crecimiento)"),
          tags$li("Balance < 0: Más parados que contratos (mercado en contracción)"),
          tags$li("Balance = 0: Equilibrio en el mercado laboral")
        )
      )
    )
  })
